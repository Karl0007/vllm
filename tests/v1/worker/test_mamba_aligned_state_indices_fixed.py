...
def test_ensure_align_ctx_does_not_keep_the_capture_time_tables():
    """A capture/dummy batch must not decide which tables later launches read.
    ...
    1. ``capture_model`` -> ``cudagraph_utils`` builds capture metadata with
       ...
       ``_ensure_align_ctx``, which only holds gathered views: it launches
       against a temporary binding (``temporary=True``) and releases it.
    2. On the first real batch ``preprocess_state`` offers the SOURCE
       per-request-slot tables
       (``tuple(bt.gpu for bt in self.block_tables.block_tables)``,
       model_runner.py:1723) and binds those.

    Dummy runs skip ``preprocess_state`` entirely, so a context that kept the
    capture-time binding would never be corrected.
    """
    device = torch.device("cuda")
    source_tables = _make_source_block_tables(device)
    gathered_tables = [torch.zeros_like(t) for t in source_tables]
    ...
    # 1. capture time: the launch runs against the gathered views, and the
    # binding is temporary -- ``prepare_attn`` releases it after the launch.
    ctx, temporary = state._ensure_align_ctx(
        kv_cache_config, group_ids, tuple(gathered_tables)
    )
    assert temporary, "a capture/dummy batch claimed the persistent binding"
    capture_out = _run(ctx, seq_lens, idx_mapping, num_reqs, device).cpu()
    assert (capture_out == 0).all(), "expected the zeroed capture-time tables"
    ctx.is_initialized = False  # what prepare_attn does after its launch

    # 2. first real batch: the source per-request-slot tables bind the context.
    ctx2, temporary2 = state._ensure_align_ctx(
        kv_cache_config, group_ids, tuple(source_tables), bind=True
    )
    assert ctx2 is ctx
    assert not temporary2
    assert ctx.is_initialized
    assert ctx.block_table_ptrs.tolist() == [
        _as_i64(t.data_ptr()) for t in source_tables
    ], "the context did not bind the source per-request-slot tables"

    # And the launch now reads source-slot rows through idx_mapping.
    out = _run(ctx, seq_lens, idx_mapping, num_reqs, device).cpu()
    assert torch.equal(out, _oracle(seq_lens, idx_mapping, num_reqs))
    assert not torch.equal(out, capture_out)
